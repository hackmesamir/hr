...
export default function EditLeave({ leave, users, leaveTypes, statuses, pendingLeavesCount = 0 }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    put(`/admin/leaves/${leave.id}`);
  };

  return (
    <div className="min-h-screen bg-gray-50">
      <Head title={`Edit Leave Request - ${leave.user.name}`} />
      ...
      <main className="md:ml-64 min-h-screen transition-all duration-300 px-4 py-6 md:px-6 lg:px-8">
        <div className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Edit Leave Request</h1>
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Update leave request details</p>
          </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Update the leave request details</CardDescription>
          </CardHeader>
